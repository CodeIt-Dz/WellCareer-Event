                ...
                s => {
                    return (
                        <div {...props} className={clsx("flex flex-col gap-8 font-semibold   ", props.className)} >
                            <h1 className="text-5xl font-bold m-3">Nos services</h1>
                            {
                                props.tabs.map((tab, index) => (
                                    <motion.button
                                        key={index}
                                        onClick={() => s.setState(index)} className={clsx(s.state == index ? "bg-primary text-white" : "bg-white", " text-3xl  decoration-transparent border-none outline-none rounded-full text-start px-4 py-3 duration-200  transition-all")}>
                                        {tab}
                                    </motion.button>
